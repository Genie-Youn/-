			...
			.add(
					"doksoft_maps",
					{
						...
						init : function(h) {
							...
							CKEDITOR.config.doksoft_maps_width = h.config.doksoft_maps_width || 400;
							CKEDITOR.config.doksoft_maps_height = h.config.doksoft_maps_height || 320;
							CKEDITOR.config.doksoft_maps_default_x = h.config.doksoft_maps_default_x
									|| 37.563882;
							CKEDITOR.config.doksoft_maps_default_y = h.config.doksoft_maps_default_y || 126.964922;
							CKEDITOR.config.doksoft_maps_default_zoom = h.config.doksoft_maps_default_zoom || 10;
							CKEDITOR.config.doksoft_maps_auto_scaling_on_search = h.config.doksoft_maps_auto_scaling_on_search || true;
							h.path = this.path;
...
//											});
							...
							this.unprotectSource = function(n) {
								if (!/<script[^>]*class[\s]*=[\s]*"doksoft_maps_google"[^>]*>/gi
										.test(n)) {
									hs += '<script class="doksoft_maps_google" src="http://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places,weather"><\/script>';
									hs += '<script class="doksoft_maps_google" src="http://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerwithlabel/src/markerwithlabel_packed.js"><\/script>';
								}
								if (!/<script[^>]*class[\s]*=[\s]*"doksoft_maps_loadmap"[^>]*>/gi
										.test(n)) {
									hs += '<script class="doksoft_maps_loadmap">'
								...
								var o = n
										.replace(
												...
												function(p, q) {
													h.plugins.doksoft_maps.softed = 1;
													e++;
